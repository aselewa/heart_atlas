...
palette <- setNames(brewer.pal(4, "Set2"), unique(srna$region))
p1 <- custom_dim_plot(seurat = srna, group.by = 'region', palette = palette, legend = F, label = F, pt.size = 0.05) + ggtitle('snRNA-seq') 
p2 <- custom_archr_plot(archr_project = satac, group.by = 'regions', palette = palette, legend = F, label = F, pt.size = 0.05) + ggtitle('snATAC-seq')
p1 + p2
```
