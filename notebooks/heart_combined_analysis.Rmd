...
Both methods generally detect the same peaks differentially accessible peaks despite having different normalization strategies. edgeR normalizes by the total number of fragments in the pseudo-bulk library, while ArchR normalizes by the number of reads in TSS only. Below is a plot of the log fold change from edgeR and from ArchR. 
...
Overlap peaks with top eQTLs

```{r}
vent.specific.eqtls <- plyranges::join_overlap_inner(vent.specific, eqtl.gr)
vent.shared.eqtls <- plyranges::join_overlap_inner(vent.shared, eqtl.gr)
```

Pull genomic annotations

```{r}
genomic.annots <- rtracklayer::import('/project2/xinhe/alan/hg38_annotations/gencode.v27.w.introns.gtf', format = 'gtf')
genomic.annots <- genomic.annots[!(genomic.annots$type %in% c("transcript","gene","CDS")),]
genomic.annots <- genomic.annots[genomic.annots$source=="HAVANA",]
eqtl.annot <- plyranges::join_overlap_inner(eqtl.gr, genomic.annots)
```

Overlap eQTLs with exons, introns, etc.

```{r}
annot.freq <- as.data.frame(eqtl.annot@elementMetadata) %>% group_by(SNP) %>% count(type) %>% group_by(SNP) %>% mutate(f = n / sum(n))

eqtlsIn <- length(unique(eqtl.annot$SNP))
nIntergenic <- length(eqtl.gr) - eqtlsIn
eqtl.dist <- c("intergenic"=nIntergenic)
eqtl.dist["exon"] <- sum(annot.freq$f[annot.freq$type=="exon"])
eqtl.dist["UTR"] <- sum(annot.freq$f[annot.freq$type=="UTR"])
eqtl.dist["Specific Vent CM"] <- length(vent.specific.eqtls)
eqtl.dist["Shared Vent CM"] <- length(vent.shared.eqtls)
eqtl.dist["intronic"] <- sum(annot.freq$f[annot.freq$type=="intron"])

eqtl.dist.df <- data.frame(freq=100*eqtl.dist/length(eqtl.gr), category=factor(names(eqtl.dist), levels=c("exon","intronic","UTR","intergenic","Specific Vent CM","Shared Vent CM")))
```
...
```{r}
ggplot(eqtl.dist.df, aes(y=category, x=freq)) + geom_bar(stat="identity") + ggClean() + xlab("Percent") + ylab("Category") + ggtitle("Distribution of Top eQTLs")
```
